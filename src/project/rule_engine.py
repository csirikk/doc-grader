...
class RuleEngine:
	...
	def __init__(self, *, high_confidence_threshold: float | None = None):
		self.high_confidence_threshold = (
			high_confidence_threshold if high_confidence_threshold is not None else HIGH_CONFIDENCE_THRESHOLD
		)
	...
	def process(self, batches: Iterable[List[Finding]]) -> tuple[List[Finding], Dict]:
		...
		for seq in batches:
			for f in seq:
				# Confidence filter
				if f.confidence is not None and f.confidence < self.high_confidence_threshold:
					dropped_low_conf += 1
					continue
