...
def _classify_text_block(
    block: Dict[str, Any], avg_font_size: float
) -> tuple[str, int | None]:
    """
...
def _bboxes_collide(bbox1: BBox, bbox2: BBox, proximity: float = 20.0) -> bool:
    ...
    return (
        bbox1.x1 + proximity >= bbox2.x0
        and bbox2.x1 + proximity >= bbox1.x0
        and bbox1.y1 + proximity >= bbox2.y0
        and bbox2.y1 + proximity >= bbox1.y0
    )
...
def _cluster_drawings_by_proximity(
    drawings: List[Dict[str, Any]], proximity: float = 20.0
...
def _handle_image_block(
    path: Path, page_index: int, bbox: BBox, out: List[Any], page
) -> None:
...
def _handle_vector_drawings(
    path: Path,
    page_index: int,
    page,
    out: List[Any],
    *,
    min_cluster_area: float = 100.0,
    ...
    debug(
        "parse_pdf: found %d vector drawings (excluding hairlines)",
        len(filtered_drawings),
    )
        ...
        for page_index in range(page_count):
            ...
            for block in page_blocks:
                ...
                if btype == 0:
                    ...
                    debug(
                        "parse_pdf: skipping unknown block type %s on page %d",
                        btype,
                        page_index + 1,
                    )
                    continue
